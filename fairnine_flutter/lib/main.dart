...
import 'native.dart'; // Import Rust bridge wrapper
...
class _HUDOverlayState extends State<HUDOverlay>
  ...
  void initState() {
    ...
    // Initialize Rust stream
    try {
      _transcriptionStream = api.createTranscriptionStream();
      _transcriptionStream?.listen((text) {
          if (text.isNotEmpty) {
             setState(() => _ghostText = text);
             // Simple debounced solidify simulation or direct stream handling could be added here
             // For now just update ghost text
             _solidifyGhostText();
          }
      });
    } catch (_) {}
    ...
    if (_isRecording) {
      ...
      try {
        if (_sttMode == STTMode.batch) {
            api.startBatchRecording();
        } else {
            // Streaming starts automatically usually, but we ensure it's on
            // The stream listener above handles the UI updates
        }
      } catch (e) {
      ...
      if (_sttMode == STTMode.batch) {
        ...
        // Call Rust backend to stop and transcribe
        api.stopAndTranscribe().then((result) {
            if (mounted) {
                setState(() {
                  _batchResult = result;
             ...
             print("Batch transcription error: $e");
        });
  ...
  void _syncPaste() {
    ...
    if (text.isNotEmpty) {
      // Call Rust backend to inject text
      try {
        api.injectText(text: text, delayMs: _typingDelay.inMilliseconds);
      } catch (e) {
  ...
  void _pasteBatch() {
    if (_batchResult.isNotEmpty) {
      // Call Rust backend to inject text
      try {
        api.injectText(text: _batchResult, delayMs: _typingDelay.inMilliseconds);
      } catch (e) {
        print("Injection error: $e");
      }
  ...
  Future<void> _loadSnippets() async {
    try {
      // Load from Rust backend? Or just file?
      // For now keeping local JSON logic as fallback or sync
      final directory = await getApplicationSupportDirectory();
  ...
  Future<void> _saveSnippets() async {
    try {
      ...
      // Also sync to Rust backend if needed (though Rust reads the same file path)
      // api.loadSnippets(); 
    } catch (_) {}
  }
  ...
  void _addSnippet() {
    ...
    // Rust side update
    try {
        api.addSnippet(trigger: trigger, content: content);
    } catch (_) {}
  ...
  void _removeSnippet(int index) {
    ...
    // Rust side update
    try {
        api.removeSnippet(trigger: trigger!);
    } catch (_) {}
  ...
  Future<void> _checkOllamaStatus() async {
    try {
        // Verify with Rust backend if possible, or keep direct HTTP
        // Keeping HTTP for now as fail-safe
  ...
  Future<void> _activateCommandMode() async {
    ...
    try {
      ...
      // Simulate Ctrl+C via platform channel or native logic
      // Ideally calls api.triggerCopy() but for now we rely on clipboard access
    ...
    // Start Rust batch recording
    try {
        api.startBatchRecording();
    } catch (_) {}

    // Wait for user to release (simulated with a delay for now)
    // In real app, user clicks "Stop" or holds key.
    // For this flow, we'll auto-stop after 3s to simulate a short command
    await Future.delayed(const Duration(seconds: 3));
    
    try {
        _commandVoiceInput = await api.stopAndTranscribe();
    } catch (e) {
        _commandVoiceInput = "Fix grammar"; // Fallback mock
    }
    ...
    // Step 3: Send to Ollama via Rust
    setState(() => _commandStatus = 'AI processing: "$_commandVoiceInput"');

    try {
       final result = await api.processAiCommand(selectedText: _selectedText, voiceCommand: _commandVoiceInput);
       ...
       // Use Rust injection
       await api.injectText(text: result, delayMs: _typingDelay.inMilliseconds);
